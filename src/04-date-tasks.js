...
function timeSpanToString(startDate, endDate) {
  ...
  let seconds = (diff - (hours * 3600000) - (minutes * 60000) - milli) / 1000;
  if (hours < 10) hours = `0${hours}`;
  if (minutes < 10) minutes = `0${minutes}`;
  if (seconds < 10) seconds = `0${seconds}`;
  if (milli < 99 && milli > 9) milli = `0${milli}`;
...
function angleBetweenClockHands(date) {
  ...
  const diff = Math.abs(0.5 * (60 * hours - 11 * minutes)) % 360;
  if (diff === 180) return diff * (Math.PI / 180);
  if (diff === 0) return 0;
  if (diff % 180 !== 0 && diff < 180) return (diff % 360) * (Math.PI / 180);
  if (diff % 180 !== 0 && diff > 180 && diff <= 270) return (diff % 180) * (Math.PI / 180);
